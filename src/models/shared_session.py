...
class AnalysisSnapshot:
    ...
    validity_records: tuple[tuple[int, Optional[bool]], ...]
    validity_state_records: tuple[tuple[int, str], ...] = ()
    validity_source_records: tuple[tuple[int, str], ...] = ()
    validity_result_records: tuple[tuple[int, int], ...] = ()
...
class LapValidityData:
    ...
    result_id: Optional[int] = None  # In-memory completed LapData identity
    ...
    def get_analysis_snapshot_for_origin(
        ...
        with self._lock:
            ...
            validity_records = tuple(
                (lap_num, None if validity.lap_state == "UNVERIFIED" else bool(validity.is_valid))
            ...
            snapshot = AnalysisSnapshot(
                ...
                validity_result_records=tuple(
                    (number, value.result_id)
                    for number, value in sorted(self._session_data.lap_validity.items())
                    if value.result_id is not None
                ),
                validity_source_records=tuple(
                    (number, value.source)
                    for number, value in sorted(self._session_data.lap_validity.items())
                ),
                validity_state_records=tuple(
                    (number, value.lap_state)
                    for number, value in sorted(self._session_data.lap_validity.items())
                ),
            )
            return OriginSnapshotResult(relation, current, snapshot)
