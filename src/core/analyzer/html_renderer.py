...
async def render_html(
    ...
    for lap in data["laps"]:
        ...
        laps_json.append(
            ...
            "is_valid": lap.get("is_valid") is True and not lap.get("is_unverified"),
            "is_unverified": bool(lap.get("is_unverified") or lap.get("is_valid") is None),
...
function makeLapFilters(containerId, onChange) {
  ...
  DATA.laps.forEach(lap => {
    ...
    const validity = lap.is_unverified ? ' [UNVERIFIED]' : lap.is_valid ? '' : ' [INVALID]';
  ...
  if (notice && (DATA.analysis_mode !== 'full' || notes.length)) {
    ...
    if (DATA.analysis_mode !== 'full') {
      notice.appendChild(document.createTextNode(
        ' Detailed coaching is unavailable for this session. See the reasons below.'
      ));
    }
    if (notes.length) {
...
window.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    DATA.laps.forEach(l => {
      ...
      opt.value = l.lap_num;
      opt.textContent = `Lap ${l.lap_num}${l.lap_num===DATA.best_lap_num?'*':''} (${l.lap_time_str})${l.is_unverified?' [UNVERIFIED]':l.is_valid?'':' [INVALID]'}`;
      sel.appendChild(opt);
    });
