...
def _unverified_lap_lines(ctx: PromptContext) -> List[str]:
    if not ctx.unverified_laps:
        return []
    return [
        "", "UNVERIFIED LAPS (diagnostic only; excluded from coaching and personal bests):",
        *(f"- Lap {lap['lap_num']}: {lap['lap_time_str']} [UNVERIFIED]" for lap in ctx.unverified_laps),
    ]
...
def build_diagnostic_sections(ctx: PromptContext) -> List[str]:
    ...
    if ctx.no_valid_laps:
        lines.append(
            "Use this session only for diagnostics; record at least one verified valid lap for coaching."
            if ctx.unverified_laps else
            "Use this session only for invalid-lap diagnostics; record at least one valid lap for coaching."
        )
    else:
        lines.append("Use this session only for diagnostics; no coaching conclusions should be drawn from it.")
    return lines + _unverified_lap_lines(ctx)
...
def build_single_lap_sections(ctx: PromptContext) -> List[str]:
    ...
    lines.extend(_unverified_lap_lines(ctx))
    if ctx.analysis_notes:
