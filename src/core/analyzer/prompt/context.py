...
class PromptContext:
    ...
    def from_data(cls, data: Mapping[str, Any]) -> "PromptContext":
        ...
        hz = data.get("hz", 10.0)
        unverified_laps = tuple(
            lap for lap in all_laps
            if lap.get("is_unverified") or lap.get("is_valid") is None
        )
        valid_laps = tuple(
            lap for lap in all_laps
            if lap.get("is_valid") is True and lap not in unverified_laps
        )
        ...
        invalid_laps = tuple(
            lap for lap in all_laps if lap.get("is_valid") is False and lap not in unverified_laps
        )
        ...
        return cls(
            ...
            unverified_laps=unverified_laps,
            best_lap=best_lap,
