...
class SubmittedLapSnapshot:
    """Actual result fields handed to transport, not the mutable current lap."""
...
class SubmissionResult:
    ...
    submitted_snapshot: Optional[SubmittedLapSnapshot] = None
...
class APIClient:
    ...
    async def submit_lap(
        ...
        if lap.is_unverified:
            return SubmissionResult(
                SubmissionStatus.UNVERIFIED_LAP, "Waiting for lap-validity data"
            )
        if lap.lap_type == "OUTLAP":
            return SubmissionResult(SubmissionStatus.INVALID_LAP, "Outlap is not a completed result")

        # The parser has reconciled the owned completion, preferring the
        # authoritative log verdict. An unrelated current-lap SHM flag must
        # never replace that completed result at the submission boundary.
        ...
        # Preparing transport may yield to a delayed verdict or session change.
        # Re-read all eligibility and ownership inputs after that await, then
        # sign and dispatch without another application-level yield in between.
        try:
            prepared_client = await self._get_client()
        except (httpx.NetworkError, httpx.TimeoutException, RuntimeError, OSError, ConnectionError) as exc:
            return self._transport_error(exc)
        if not is_secret_configured():
            return SubmissionResult(SubmissionStatus.NO_SECRET, "APP_SECRET not configured")
        if lap.is_unverified:
            return SubmissionResult(SubmissionStatus.UNVERIFIED_LAP, "Waiting for lap-validity data")
        if lap.lap_type == "OUTLAP":
            return SubmissionResult(SubmissionStatus.INVALID_LAP, "Outlap is not a completed result")
        if is_game_running() != GameProcessStatus.RUNNING:
            return SubmissionResult(SubmissionStatus.GAME_NOT_RUNNING, "Game must be running to submit laps")
        effective_is_valid = lap.is_valid
        final_user_id, shared_snapshot, preflight_error = self._validate_submission_preflight(
            session=session, effective_is_valid=effective_is_valid,
            # A previously frozen owner identity remains safe if the manager
            # has rolled to another car while transport was being prepared.
            user_id=user_id or final_user_id, submit_invalid=submit_invalid, lap_number=lap.lap_number,
        )
        if preflight_error is not None:
            return preflight_error
        if final_user_id is None:
            return SubmissionResult(SubmissionStatus.ERROR, "No Steam ID detected - please start a session in game")
        payload, payload_error = self._build_submission_payload(
            session=session, lap=lap, final_user_id=final_user_id,
            shared_snapshot=shared_snapshot, effective_is_valid=effective_is_valid,
        )
        if payload_error is not None:
            return payload_error
        if payload is None:
            return SubmissionResult(SubmissionStatus.ERROR, "Unable to build lap submission payload")
        submitted_snapshot = SubmittedLapSnapshot(
            is_valid=bool(payload["valid"]), lap_time_ms=int(payload["time"])
        )
        ...
        transport_result = await self._send_submission(signed_payload, client=prepared_client)
        if isinstance(transport_result, SubmissionResult):
            result = transport_result
        else:
            result = self._map_submission_response(transport_result, signed_payload)
        result.submitted_snapshot = submitted_snapshot
        return result
    ...
    @staticmethod
    def _transport_error(exc: Exception) -> SubmissionResult:
        if isinstance(exc, httpx.NetworkError):
            detail = str(exc).strip() or type(exc).__name__
            log_error(Component.API, "Network error", error=detail)
            return SubmissionResult(SubmissionStatus.NETWORK_ERROR, f"Network error: {detail}")
        if isinstance(exc, httpx.TimeoutException):
            log_error(Component.API, "Request timeout")
            return SubmissionResult(SubmissionStatus.NETWORK_ERROR, "Request timed out")
        log_exception(Component.API, "API exception", exc)
        return SubmissionResult(SubmissionStatus.ERROR, f"Unexpected error: {exc}")
