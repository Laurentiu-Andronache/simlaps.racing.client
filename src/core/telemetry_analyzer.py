    ...
    async def analyze(
        ...
        if same_origin and shared_snapshot is not None:
            if isinstance(shared_snapshot, AnalysisSnapshot):
                ...
                shared_validity_sources = dict(shared_snapshot.validity_source_records)
                shared_validity_states = dict(shared_snapshot.validity_state_records)
                shared_validity_results = dict(shared_snapshot.validity_result_records)
            else:
                shared_lap_times = {
                ...
                shared_lap_validity = {
                    lap_num: None if validity.lap_state == "UNVERIFIED" else validity.is_valid
                    for lap_num, validity in shared_snapshot.lap_validity.items()
                }
                shared_validity_sources = {
                    lap_num: validity.source
                    for lap_num, validity in shared_snapshot.lap_validity.items()
                }
                shared_validity_states = {
                    lap_num: validity.lap_state
                    for lap_num, validity in shared_snapshot.lap_validity.items()
                }
                shared_validity_results = {
                    lap_num: validity.result_id
                    for lap_num, validity in shared_snapshot.lap_validity.items()
                }
        else:
            shared_lap_times = {}
        ...
        # Speed is not a timing boundary: the first timed lap may include a
        # long stationary interval or a standing start. Keep its full timer
        # epoch; completed-lap cleaning below owns any stale-prefix removal.
        track = build_track(frames, hz=hz)
        if not track:
        ...
        if game_lap_boundaries and len(game_lap_boundaries) >= 1:
            ...
            if isinstance(game_lap_boundaries[0], (tuple, list)):
                ...
                sorted_markers = sorted(
                    (
                        (
                            ...
                            str(b[3]) if len(b) > 3 and b[3] is not None else "UNVERIFIED",
                            getattr(b, "result_id", None),
                        )
                ...
                if timing_bounds:
                    ...
                    for idx, frame in enumerate(timing_bounds):
                        ...
                        reconciled_markers.append(
                            (
                                ...
                                match[3] if match else "UNVERIFIED",
                                source_lap_num,
                                match[4] if match else None,
                            )
                        )
                    ...
                    lap_result_ids = [marker[5] for marker in reconciled_markers]
                    ...
                    lap_result_ids = [marker[4] for marker in sorted_markers]
                    reserved_explicit_lap_numbers = {
                        marker[2] for marker in sorted_markers if marker[2] is not None
                    }
                    used_shared_lap_numbers = set(reserved_explicit_lap_numbers)
        ...
        for i in range(len(lap_bounds) - 1):
            ...
            lap_type = lap_types[i] if lap_types and i < len(lap_types) else "UNVERIFIED"
            result_id = lap_result_ids[i] if lap_result_ids and i < len(lap_result_ids) else None
            if lap_type in {"INLAP", "ABORTED"}:
                continue
            ...
            # A callback with no verdict cannot inherit a live in-progress
            # flag merely because the lap ordinal happens to match. Late
            # reconciliation needs this capture's owner and a completed time.
            owned_completed_verdict = (
                capture_origin is not None
                and same_origin
                and source_lap_num is not None
                and shared_validity_sources.get(source_lap_num) == "logs"
                and (
                    result_id is None
                    or shared_validity_results.get(source_lap_num) == result_id
                )
                and _nearest_lap_marker_by_time(
                    [(source_lap_num, shared_lap_times.get(source_lap_num))],
                    game_lap_time_ms,
                ) is not None
            )
            if lap_type == "OUTLAP":
                # A delayed official verdict can correct the initial pit-exit
                # classification. Reuse this exact physical interval, but only
                # a frozen valid completion belonging to its owner can turn
                # a structural boundary into a reportable timed lap.
                if not (
                    owned_completed_verdict
                    and authoritative_validity is True
                    and shared_validity_states.get(source_lap_num) == "VALID"
                ):
                    continue
                lap_type = "VALID"
            if lap_type == "UNVERIFIED" and not owned_completed_verdict:
                authoritative_validity = None
            is_unverified = lap_type == "UNVERIFIED" and not isinstance(authoritative_validity, bool)
        ...
        if shared_lap_times and laps:
            try:
                ...
                pass
        valid_laps = [lap for lap in laps if lap.get("is_valid") is True and not lap.get("is_unverified")]
        ...
        if best_lap is None:
            analysis_mode = "diagnostic"
            analysis_notes.append(
                "No verified valid completed laps were available; other laps are shown for diagnostics only."
                if any(lap["is_unverified"] for lap in laps)
                else "No valid completed laps were available; invalid laps are shown for diagnostics only."
            )
