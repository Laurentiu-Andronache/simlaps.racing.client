...
Changes from v2:
  1.  LapState enum  — replaces scattered `lap_type` string derivation
  2.  Sector consistency guard  — |S1+S2+S3 − lap_time| ≤ 50 ms
  3.  physics_lap_number stored in LapData for external validation
  4.  physics_lap_num == 1 used as practice outlap fallback signal
  5.  Split key guard  — keys must be exactly {0, 1, 2}, order independent
  6.  S1 corruption uses `s1 > lap_time_ms` (not fixed 60 s threshold)
  7.  Per-lap fuel reliability; session unreliable only after N spikes
  8.  Stint detection  — compound change triggers a new stint
  9.  Aborted lap emitted when a non-empty in-progress lap is abandoned
  10. StintData dataclass added to SessionData
"""
...
# ─── Debug logger (singleton, disabled in production) ────────────────────────
...
class LapData:
    ...
    lap_state: LapState = LapState.PUSH
