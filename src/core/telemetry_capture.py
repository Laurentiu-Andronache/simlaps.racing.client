...
from src.models import LapData, SessionOriginSnapshot, SharedSessionManager, car_models_match
...
class TelemetryCapture:
    ...
    def __init__(
        ...
        self._lap_boundary_bindings: dict[tuple[str, int], tuple[Any, int]] = {}
    ...
    def set_record_frames(self, record: bool) -> None:
        ...
        if was_recording and not record:
            ...
            self._lap_boundary_bindings.clear()
            self._recording_awaiting_boundary = False
    ...
    def _capture_origin_ready(self) -> bool:
        ...
        return self._origin_matches_capture(self._get_manager_origin())

    def _origin_matches_capture(self, current: Optional[SessionOriginSnapshot]) -> bool:
        """Validate the exact immutable snapshot subsequently used for writes."""
    ...
    def record_lap_boundary(
        ...
        self._lap_boundaries.append(LapBoundary(frame_idx, lap_time_ms, lap_number, lap_type, lap_number))
        log_info(
            Component.TELEMETRY,
    ...
    def bind_lap_boundary(self, session_id: str, lap: LapData) -> bool:
        """Bind the just-recorded marker to its result, independent of numbering."""
        key = (session_id, id(lap))
        if key in self._lap_boundary_bindings or not self.owns_session(session_id) or not self._lap_boundaries:
            return False
        index = len(self._lap_boundaries) - 1
        boundary = self._lap_boundaries[index]
        if (
            boundary.frame_index != len(self._frames) - 1
            or boundary.lap_number != lap.lap_number
            or boundary.lap_time_ms != lap.lap_time_ms
        ):
            return False
        self._lap_boundary_bindings[key] = (
            ref(lap, lambda _ref: self._lap_boundary_bindings.pop(key, None)), index,
        )
        self._lap_boundaries[index] = boundary._replace(result_id=id(lap))
        return True

    def reconcile_lap_boundary(self, session_id: str, lap: LapData) -> bool:
        """Correct one result's marker metadata without moving its physical boundary.

        Preserve the originally observed classification: analysis resolves a
        late verdict from the owned completed snapshot, never from a mutable
        live flag. Equal-time neighboring laps cannot share this binding.
        """
        binding = self._lap_boundary_bindings.get((session_id, id(lap)))
        if binding is None or binding[0]() is not lap or not self.owns_session(session_id):
            return False
        index = binding[1]
        if index >= len(self._lap_boundaries):
            return False
        boundary = self._lap_boundaries[index]
        self._lap_boundaries[index] = boundary._replace(
            lap_time_ms=lap.lap_time_ms, lap_number=lap.lap_number,
        )
        return True
    ...
    def _capture_frame(self, frame_num: int) -> FrameData:
        ...
        origin_stable_after_graphics = self._origin_matches_capture(expected_origin)
        if origin_stable_after_graphics:
            self._confirm_capture_origin(expected_origin)
        origin_stable = (
        ...
        try:
            while self._running:
                ...
                if self._last_sample_had_data:
                    ...
                    # A stationary car can still own a live timed lap. Keep
                    # its frames and the validity reader running until an
                    # actual lifecycle stop or loss of the game/mappings.
