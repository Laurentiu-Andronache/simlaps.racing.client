...
async def test_cold_start_does_not_apply_unrelated_shared_validity_key(tmp_path):
    ...
    # Neither unmatched callback nor unrelated live key 5 proves these
    # physical completions valid or game-invalid. Keep the official results.
    assert [lap["lap_time_s"] for lap in data["laps"]] == [10.0, 11.0]
    assert [lap["is_valid"] for lap in data["laps"]] == [False, False]
    assert [lap["is_unverified"] for lap in data["laps"]] == [True, True]
